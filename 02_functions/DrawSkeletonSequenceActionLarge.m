...
function h=DrawSkeletonSequenceActionLarge(skeleton_bis,k,Ec,Lc,fs,varargin)
    ...
    for i=1:k:m
        ...
        cnt = cnt + 1;
        stepp = ss*(i-1)/k; 
        plot3(skeleton_bis(1:n,i,1)+stepp,skeleton_bis(1:n,i,2),skeleton_bis(1:n,i,3)+zoffset ,'.','MarkerSize',35,'MarkerFaceColor',Lc,'MarkerEdgeColor',Lc);
        hold on
        c = mean(skeleton_bis(1:n,i,1)+stepp);
        %text(c, 0, 1, strcat('t=',num2str(cnt)));
        for j=1:length(a)
            line([skeleton_bis(a(j),i,1)+stepp skeleton_bis(b(j),i,1)+stepp],[skeleton_bis(a(j),i,2) skeleton_bis(b(j),i,2)],[skeleton_bis(a(j),i,3) skeleton_bis(b(j),i ,3)]+zoffset,'Color',Ec,'LineWidth',5);
            axis equal
            axis off
            hold on,
          %  view([-182 -82])
    ...
    if nargin > 7
        lbl = varargin{3};
        text(mean(squeeze(skeleton_bis(1:n,1,1)))-2, mean(squeeze(skeleton_bis(1:n,1,2))), mean(squeeze(skeleton_bis(1:n,1,3)))+zoffset, lbl,'FontSize',fs);
    end
end
