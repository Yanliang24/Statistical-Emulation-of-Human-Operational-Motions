function h=DrawSkeletonSequenceAction_1(skeleton_bis,k,Ec,Lc,varargin)
    ...
    hh = max(max(squeeze(skeleton_bis(1:n,:,3)))) +0.1;
    ...
    for i=1:k:m
        ...
        if nargin > 7,
            lbl = varargin{4};
            lbl = strcat(sprintf('%3d', lbl(i)));
            text(mean(squeeze(skeleton_bis(1:n,i,1)))+stepp, mean(squeeze(skeleton_bis(1:n,i,2))), hh+zoffset, lbl, 'Interpreter','latex','FontSize',16);
        end
        pause(0.01);
    end
    ...
    if nargin > 6
        lbl = varargin{3};
        text(mean(squeeze(skeleton_bis(1:n,1,1)))-1, mean(squeeze(skeleton_bis(1:n,1,2))), mean(squeeze(skeleton_bis(1:n,1,3)))+zoffset, lbl);
    end
end
